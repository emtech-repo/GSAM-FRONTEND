...
export class LoginComponent implements OnInit {
  ...
  isBypassMode: boolean = true; // Initialize with true or false as needed
      ...
      setTimeout(() => {
        ...
        if (mockResponse.statusCode === 200) {
          ...
          localStorage.setItem('currentUser', JSON.stringify({
            token: mockResponse.result.token,
            role: mockResponse.result.roles,
            email: mockResponse.result.email
          }));
          ...
          showSuccessToast(mockResponse.message);
        } else {
          // Show error message
          showDangerToast(mockResponse.message);
        }
